...
class TransactionsApi {
  ...
  Future<BatchCheckInResponse> submitBatchCheckIn({
    ...
    if (checkIns.isEmpty) {
      throw CheckInValidationException('check_ins must not be empty.');
    }
    if (AppConfig.useStubApi) {
      return BatchCheckInResponse.stubForCheckIns(checkIns);
    }
  ...
  Future<CheckInResponse> submitCheckIn({
    ...
    if (vr == null || vr.isEmpty) {
      throw CheckInValidationException('vr_no is required.');
    }

    final vehicleClean = Map<String, dynamic>.from(vehicle);
    vehicleClean.remove('vr_no');
    vehicleClean.remove('vrNo');
    if (vehicleClean['plate_number'] != null) {
      vehicleClean['plate_number'] =
          normalizePlateNumber(vehicleClean['plate_number'].toString())
              .toUpperCase();
    }
